...
export async function insertS(student) {
    try {
        ...
        const insertRes = await db.executeSql(
            ...
            (name, surname, phone, email, form, platform, nick, city, address, disabled)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                student.name,
                student.surname,
...
export async function importS(array) {
    try {
        ...
        const db = await SQLite.openDatabase({ name: 'studentlog.db', location: 'default' })
        const items=array.map(x=>`(${x.id}, "${x.name}", "${x.surname}", "${x.phone}", "${x.email}", ${x.form}, "${x.platform}", "${x.nick}", "${x.city}", "${x.address}", ${x.disabled})`).join(",");
        const insertRes = await db.executeSql(
            `INSERT INTO students 
            (id, name, surname, phone, email, form, platform, nick, city, address, disabled)
            VALUES ${items}`);
    } catch (error) {
        // console.error(`Error while inserting a new student: ${JSON.stringify(error)}`);
        return error;
